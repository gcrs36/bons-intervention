...
class DolibarrClient {
  ...
  async request(method, resource, body) {
    ...
    try {
      ...
      if (!response.ok) {
        const apiMessage = data?.error?.message || data?.error || data?.message || (typeof data === 'string' ? data : '');
  ...
  async searchThirdparties(query) {
    ...
    const result = await this.request('GET', `thirdparties?${params}`);
...
async function buildDolibarrFlow({ client, bon, pdfPath, onProgress = async () => {} }) {
  ...
  if (!ids.thirdparty) {
    ...
    ids.thirdparty = exact?.id || Number(await client.request('POST', 'thirdparties', {
      name: bon.client, client: 1, email: bon.mail || '', phone: bon.tel_ || '', address: bon.adresse || '',
...
module.exports = { DolibarrClient, DolibarrError, buildDolibarrFlow, durationSeconds, dolibarrLine };
