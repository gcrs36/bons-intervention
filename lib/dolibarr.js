  ...
  async request(method, resource, body) {
    ...
    try {
      ...
      try {
        ...
        // documents/upload après avoir déjà enregistré le fichier. Si les
        // en-têtes confirment un succès, le corps de réponse n'est pas utile.
        if (response.ok && method === 'POST' && normalizedResource === 'documents/upload') return null;
        throw error;
      }
...
async function buildDolibarrFlow({ client, bon, pdfPath, onProgress = async () => {} }) {
  ...
  if (pdfPath && !Number(bon.dolibarr_pdf_uploaded)) {
    ...
    try {
      ...
      try {
        await client.request('POST', 'documents/upload', gedBody);
      } catch (uploadError) {
        if (!isRetryableUploadConnectionError(uploadError)) throw uploadError;
        await delay(400);
        await client.request('POST', 'documents/upload', gedBody);
      }
      pdfMessage = `PDF signé enregistré dans la GED : GCRS-Interventions/${folderRef}.`;
    }
    await onProgress('pdf', { id: 1 }, pdfMessage);
  }
