  ...
  return new Promise((resolve, reject) => {
    ...
    function drawHeader() {
      ...
      if (logoPath && fs.existsSync(logoPath)) {
        try {
          doc.image(logoPath, margin, 8, { fit: [104, 53], align: 'left', valign: 'center' });
          doc.fillColor(COLORS.ink).font('Helvetica-Bold').fontSize(11)
            .text(definition.label, margin + 114, 22, { width: 280 });
          doc.fillColor(COLORS.muted).font('Helvetica').fontSize(7.5)
            .text(`Modèle Kizeo n° ${definition.kizeoId || 'interne'}`, margin + 114, 42, { width: 260 });
        } catch {
          doc.fillColor(COLORS.ink).font('Helvetica-Bold').fontSize(15).text(definition.variant.label, margin, 29, { width: 225 });
        }
      } else {
        doc.fillColor(COLORS.ink).font('Helvetica-Bold').fontSize(15).text(definition.variant.label, margin, 27, { width: 300 });
        doc.fillColor(COLORS.muted).font('Helvetica').fontSize(7.5)
          .text(definition.label, margin, 48, { width: 300 });
      }
      ...
      doc.fillColor(COLORS.white).font('Helvetica-Bold').fontSize(10)
        .text(definition.shortLabel, pageWidth - margin - 106, 35, { width: 100, align: 'center' });
    ...
    function addPage() {
      doc.addPage({ size: 'A4', margins: { top: 82, right: 0, bottom: 0, left: 0 } });
    }
    ...
    function sourceForm() {
      ...
      const alreadyRendered = new Set([
        ...
        'ref_cde_client', 'type_materiel_', 'machines1', 'machine', 'n_de_matricule_', 'n_de_serie', 'serie',
        'non_du_technicien', 'nom_du_signataire_', 'nom_du_signataire',
      ]);
      ...
      const flush = () => {
        ...
        entries.forEach((entry) => {
          if (entry.kind === 'table') {
            flushPair();
            drawStructuredTable(entry.label, entry.rows, entry.columns);
      ...
      for (let index = 0; index < sourceFields.length; index += 1) {
        ...
        if (/fa-table/.test(icon)) {
          ...
          while (cursor < sourceFields.length && !/fa-bookmark|fa-table/.test(sourceFields[cursor].icon || '')) {
            ...
            if (!/fa-photo|fa-gavel|fa-file-image/.test(column.icon || '')) columns.push({ id: column.id, label: column.label });
            cursor += 1;
          }
          if (/pi[eè]ce|fourniture|mat[eé]riel/i.test(sourceField.label) && Array.isArray(bon.items) && bon.items.length) {
            ...
            if (Array.isArray(rows) && rows.length) entries.push({ kind: 'table', label: sourceField.label, rows, columns });
          }
          index = cursor - 1;
          continue;
        }
    ...
    function signatures() {
      ...
      section('Signatures');
      const h1 = signatureBox(leftX, y, family === 'mise_en_service' ? 'Formateur' : 'Technicien', bon.non_du_technicien, extra.signature_technicien);
    ...
    drawHeader();
    titleBlock();
    section(family === 'intervention' ? 'Client et intervention' : 'Client et document');
    grid([
