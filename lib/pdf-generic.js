...
const GCRS_LOGO_PATH = path.join(__dirname, '..', 'public', 'logo-gcrs.png');
...
function generateGenericPdf(bon, outputPath) {
  ...
  return new Promise((resolve, reject) => {
    ...
    function drawHeader() {
      doc.save();
      const useGcrsVisitLogo = definition.id === 'visite_massicot'
        && definition.variant.id === 'gcrs'
        && fs.existsSync(GCRS_LOGO_PATH);
      if (useGcrsVisitLogo) {
        try {
          doc.image(GCRS_LOGO_PATH, margin, 9, { fit: [46, 53], align: 'center', valign: 'center' });
        } catch {
          doc.fillColor(COLORS.ink).font('Helvetica-Bold').fontSize(15).text('GCRS', margin, 29, { width: 225 });
        }
      } else {
