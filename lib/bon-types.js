const VARIANTS = {
  gcrs: { id: 'gcrs', label: 'GCRS', color: '#173b57', logo: '/logo-gcrs.png' },
  abeg: { id: 'abeg', label: 'ABEG', color: '#1b5b8c', logo: '/logo-abeg.png' },
  arboreal: { id: 'arboreal', label: 'ARBOREAL', color: '#43845b', logo: '/logo-arboreal.png' },
  dimensions: { id: 'dimensions', label: 'Dimensions', color: '#9b1b30', logo: '/logo-dimensions.png' },
  esi: { id: 'esi', label: 'ESI', color: '#5553a6', logo: '/logo-esi.jpg' },
  clementz: { id: 'clementz', label: 'Clementz', color: '#515b63' },
};

const KIZEO_SCHEMA = require('./kizeo-schema.json');
...
const SYNCED_FAMILIES = new Set(['intervention', 'visite_massicot', 'mise_en_service']);
const BON_TYPES = Object.fromEntries(KIZEO_SCHEMA.forms.map((form) => [form.id, {
  id: form.id,
  kizeoId: form.kizeoId,
  label: form.label,
  shortLabel: form.label,
  prefix: form.prefix,
  family: form.family,
  description: `${form.sourceFieldCount} champs repris du formulaire Kizeo n° ${form.kizeoId}.`,
  variants: [form.company],
  syncDefault: SYNCED_FAMILIES.has(form.family),
  checklist: form.family === 'visite_massicot' ? MASSICOT_CHECKLIST : undefined,
  fields: form.fields,
}]));
...
function getBonType(type, variant) {
  const definition = BON_TYPES[String(type || '')] || LEGACY_BON_TYPES[String(type || '')];
...
module.exports = { BON_TYPES, LEGACY_BON_TYPES, VARIANTS, MASSICOT_CHECKLIST, KIZEO_SCHEMA, publicBonTypes, getBonType };
